	...
	for(YCnt=0,YPos=y0,Pass=0;YCnt<Height;YCnt++)
	{
	 		...
	 		else
			{
				if(Cnt)
				{
					...
					//	pic_phy.draw_hline(XPos-Cnt-1,YPos,Cnt+1,*(pTrans+OldIndex));
					 	LCD_DrawLine_Color(XPos-Cnt-1,YPos,Cnt+1,Horizontal,*(pTrans+OldIndex));
					}else if(Disposal==2)
					{
					//	pic_phy.draw_hline(XPos-Cnt-1,YPos,Cnt+1,bkcolor);
						LCD_DrawLine_Color(XPos-Cnt-1,YPos,Cnt+1,Horizontal,bkcolor);
					}
					Cnt=0;
				}else
				{
					if(OldIndex>=0)
					{
						...
						LCD_DrawPoint(XPos-1,YPos,bkcolor);
					}
				}
			}
			OldIndex=Index;
		...
		if((OldIndex!=Transparency)||(Disposal==2))
		{
			...
			//	pic_phy.draw_hline(XPos-Cnt-1,YPos,Cnt+1,ColorIndex);
				LCD_DrawLine_Color(XPos-Cnt-1,YPos,Cnt+1,Horizontal,ColorIndex);
			}else 
...
void gif_clear2bkcolor(u16 x,u16 y,gif89a* gif,ImageScreenDescriptor pimge)
{
	...
	//	pic_phy.fill(x0,y0,x1,y1,color); //设定xy,的范围不能太大.
		LCD_DrawRectFill(x0,y0,x1,y1,color);
	}
