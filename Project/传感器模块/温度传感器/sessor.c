...
/*************NTC温度传感器分压电阻阻值*************************************/
#define ADC_NTC_RESI_DIV		100000.0f

/*************NTC温度传感器输出信号放大倍数*********************************/
#define ADC_NTC_AMP_FACT		1.0f
...
void lcd_app(u16 adc_data1,float temperature);
...
int Sessor(void)
{
	...
	while(1)
	{
		/* 获得测量值，读取adc 通道0, 16bit */
		adc_data1 = tlv2543_read_data(AIN0);

		volt = adc_data1*ADC_SCALE_FACT/ADC_NTC_AMP_FACT/10;

		/* 获得温度 */
		temperature = NTC3950(volt,ADC_NTC_RESI_DIV,ADC_SENSOR_VCC);/* 获得温度 */

		/* 显示 */
		lcd_app(adc_data1,temperature);
...
void Senssor_SPI_Configuration(void)
{
	...
	SPI_InitStructure.SPI_CPOL = SPI_CPOL_Low; /* SCK常低,起始电平为低 */
